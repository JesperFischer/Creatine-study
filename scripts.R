...
get_summary = function(data){
  table = data %>% 
    ...
    group_by(tid) %>% 
    summarize(n = n(),
              crimpright = round(mean(crimphøjremax),1), 
              ch_sd = round(sd(crimphøjremax)/sqrt(n),1), 
              crimpleft = round(mean(crimpvenstremax, na.rm =T),1), 
              cv_sd = round(sd(crimpvenstremax, na.rm =T)/sqrt(n),1),
              lattice = round(mean(lattice_r, na.rm = T),1),
              lattice_sd = round(sd(lattice_r, na.rm = T)/sqrt(n),1),
              lockoffleft = round(mean(ll_r, na.rm = T),1),
              ll_sd = round(sd(ll_r, na.rm = T)/sqrt(n),1),
              lockoffright = round(mean(lr_r, na.rm = T),1),
              lr_sd = round(sd(lr_r, na.rm = T)/sqrt(n),1),
              moontal = round(mean(moon.tal, na.rm = T),1),
              moontal_sd = round(sd(moon.tal, na.rm = T)/sqrt(n),1),
              Bw = round(mean(BW, na.rm = T),1),
              BW_sd = round(sd(BW, na.rm = T)/sqrt(n),1),
              pullups = round(mean(pullups_r, na.rm = T),1),
              pullups_sd = round(sd(pullups_r, na.rm = T)/sqrt(n),1),
              pinchhøjre = round(mean(pinchhøjremax, na.rm = T),1),
              pinchhøjre_sd = round(sd(pinchhøjremax, na.rm = T)/sqrt(n),1),
              pinchvenstre = round(mean(pinchvenstremax, na.rm = T),1),
